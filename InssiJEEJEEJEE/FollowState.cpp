...
MachineState(owner, stateMachine) {
	...
	stateMachine->getTimers()->addTimer("ChangeDir", 3);
	...
	dirHandler.setOffset(sf::Vector2f(-8.0f, -8.0f));
}
...
void FollowState::update(sf::Time& tpf) {
	...
	if (getStateMachine()->getTimers()->expired("FindPath")) {
		b2Vec2 playerPos = Convert::box2dToWorld(world->getPlayer()->getPosition());
	...
	b2Vec2 velo(9.5f * trans.x, 9.5f * trans.y);
